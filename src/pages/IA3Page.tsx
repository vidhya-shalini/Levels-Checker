...
import { validateIA3, ValidationResult } from '@/services/validationService';
...
const IA3Page = () => {
        ...
        setParseResult(prev => {
          ...
          const finalResult: ValidationResult = {
            ...
            errors: unfixedErrors.map(q => ({
              part: finalPartA.includes(q) ? 'A' : finalPartB.includes(q) ? 'B' : 'C',
            ...
            partAnalysis: {
              ...
              partC: tempResult.partAnalysis.partC ? {
  ...
  const handleFixAllQuestions = async () => {
    ...
    try {
      ...
      [...parseResult.partB, ...(parseResult.partC || [])].forEach(q => {
      ...
      // Find OR partners that need to be fixed for consistency
      ...
      orPairMap.forEach((pair) => {
        ...
        // If one was fixed and the other wasn't, and their levels differ
      ...
      setParseResult(prev => {
        ...
        const finalResult: ValidationResult = {
          ...
          partAnalysis: {
            ...
            partC: tempResult.partAnalysis.partC ? {
              ...tempResult.partAnalysis.partC,
              errors: partCErrors.map(q => ({ part: 'C', questionNumber: q.questionNumber, issue: q.errorMessage || 'Unknown error', suggestion: q.expectedLevel ? `Change to ${q.expectedLevel}` : 'Fix this issue' })),
              isValid: partCErrors.length === 0
            } : undefined
          },
          allErrorsFixed: unfixedErrors.length === 0
  ...
  const handleMatchTargetDistribution = async () => {
    ...
    try {
      ...
      for (let pass = 0; pass < 3 && !isWithinTolerance(projected); pass++) {
        if (projected.l456Pct > 20 + TOLERANCE) {
          ...
          for (let i = 0; i < allCandidates.length && changed < excess; i++) {
            if (!changedIds.has(allCandidates[i].id)) { addChange(allCandidates[i], 'L3'); changed++; }
          }
      ...
      setParseResult(prev => {
        ...
        const allClean = [...cleanPartA, ...cleanPartB, ...(cleanPartC || [])];
        ...
        const finalResult: ValidationResult = {
          ...
          partAnalysis: {
            ...
            partC: tempResult.partAnalysis.partC ? {
              ...tempResult.partAnalysis.partC, errors: [], isValid: true
            } : undefined
          },
          allErrorsFixed: true
        };
  ...
  const handleDownload = async () => {
    ...
    try {
      const blob = await generateHtmlDocument(parseResult, 'IA3');
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'IA3_Model_Corrected_Paper.html';
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        {showResults && parseResult && (
          <div className="space-y-6 mb-8">
            <QuestionDisplay
              part="A"
              partTitle="10 × 2 = 20 Marks (L1: 50%, L2: 50%, All COs)"
              questions={parseResult.partA}
              onFixQuestion={handleFixQuestion}
            />
            <QuestionDisplay
              part="B"
              partTitle="13 × 5 = 65 Marks (L2-L5, All COs)"
            ...
            {parseResult.partC && parseResult.partC.length > 0 && (
              <QuestionDisplay
                part="C"
                partTitle="15 × 1 = 15 Marks (L4-L6 only)"
        ...
        <div className="mt-12 bg-card rounded-xl border border-border p-6">
          <h3 className="font-display font-bold text-foreground mb-4">IA3/Model Verification Rules</h3>
          <div className="grid md:grid-cols-3 gap-6 text-sm">
            <div>
              <h4 className="font-semibold text-foreground mb-2">Part A (10×2 = 20 marks)</h4>
              <ul className="space-y-1 text-muted-foreground">
                <li>• 2 questions per CO (CO1-CO5)</li>
                <li>• L1 and L2 only</li>
                <li>• 50% L1, 50% L2</li>
                <li>• Same CO: different levels</li>
              </ul>
            </div>
            <div>
              <h4 className="font-semibold text-foreground mb-2">Part B (13×5 = 65 marks)</h4>
              <ul className="space-y-1 text-muted-foreground">
                <li>• 5 OR choice pairs (11-15)</li>
                <li>• Levels: L2, L3, L4, L5</li>
                <li>• OR pairs same level</li>
                <li>• One pair per CO (CO1-CO5)</li>
              </ul>
            </div>
            <div>
              <h4 className="font-semibold text-foreground mb-2">Part C (15×1 = 15 marks)</h4>
              <ul className="space-y-1 text-muted-foreground">
                <li>• Only L4, L5, L6 allowed</li>
                <li>• Two OR choices (16a, 16b)</li>
                <li>• Different COs required</li>
              </ul>
            </div>
          </div>
          <div className="mt-4 pt-4 border-t border-border">
            <p className="text-sm text-muted-foreground">
              <strong>Overall Distribution:</strong> L1 & L2 = 40% | L3 = 40% | L4, L5 & L6 = 20%
            </p>
          </div>
        </div>
      </main>
